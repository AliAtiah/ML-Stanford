...
def run_semi_supervised_em(x, x_tilde, z_tilde, w, phi, mu, sigma, max_iter=1000):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        # M-step
        for i in range(K):
        	phi[i] = (sum(w[:, i]) + alpha * sum(z_tilde==i)) / (x.shape[0] + alpha * x_tilde.shape[0])
        	x_tilde_i = x_tilde[z_tilde.reshape(-1,)==i]
        	mu[i] = (x.T.dot(w[:, i]) + alpha * x_tilde_i.sum(axis=0)) / (sum(w[:, i]) + alpha * sum(z_tilde==i))
        	sigma[i] = ((w[:, i][:, None] * (x-mu[i])).T.dot(x-mu[i]) + alpha * (x_tilde_i-mu[i]).T.dot(x_tilde_i-mu[i])) / (sum(w[:, i]) + alpha * sum(z_tilde==i))
