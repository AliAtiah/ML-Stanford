...
class Test_2aii(GradedTestCase):
  ...
  def test_3(self):
    ...
    self.assertTrue(np.allclose(h_student, h_solution, atol=0.01, rtol=0.10), 'Checking that hidden layer matches.')
    self.assertTrue(np.allclose(y_student, y_solution, atol=0.01, rtol=0.10), 'Checking that output layer matches.')
    self.assertTrue(np.allclose(cost_solution, cost_student, atol=0.01, rtol=0.10), 'Checking that cost matches.')
...
class Test_2b(GradedTestCase):
  ...
  def test_0(self):
    ...
    solution_grad = solution_backward_prop_regularized(data=self.train_data, labels=self.train_labels, params=params,
                                                       forward_prop_func=solution_forward_prop, reg=0.0001)
    results_match = True
    if not np.allclose(student_grad['W1'], solution_grad['W1'], atol=1, rtol=2):
      print('W1 weight did not match expected result')
      results_match = False

    if not np.allclose(student_grad['b1'], solution_grad['b1'], atol=1, rtol=2):
      print('b1 bias did not match expected result')
      results_match = False

    if not np.allclose(student_grad['W2'], solution_grad['W2'], atol=1, rtol=2):
      print('W2 weight did not match expected result')
      results_match = False

    if not np.allclose(student_grad['b2'], solution_grad['b2'], atol=1, rtol=2):
      print('b2 bias did not match expected result')
      results_match = False
  ...
  def test_1(self):
    ...
    for _ in range(10):
      # updates params
      submission.gradient_descent_epoch(self.train_data, self.train_labels, learning_rate=0.1, batch_size=5,
                                     params=student_params,
                                     forward_prop_func=solution_forward_prop,
                                     backward_prop_func=lambda a, b, c, d: submission.backward_prop_regularized(a, b,
                                                                                                             ...
                                                                                                             d,
                                                                                                             reg=0.0001))
      solution_gradient_descent_epoch(self.train_data, self.train_labels, learning_rate=0.1, batch_size=5,
                                      params=solution_params,
                                      forward_prop_func=solution_forward_prop,
                                      backward_prop_func=lambda a, b, c, d: solution_backward_prop_regularized(a,
                                                                                                               ...
                                                                                                               reg=0.0001))
    results_match = True
    if not np.allclose(student_params['W1'], solution_params['W1'], atol=1, rtol=2):
      print('W1 weight did not match expected result')
      results_match = False

    if not np.allclose(student_params['b1'], solution_params['b1'], atol=1, rtol=2):
      print('b1 bias did not match expected result')
      results_match = False

    if not np.allclose(student_params['W2'], solution_params['W2'], atol=1, rtol=2):
      print('W2 weight did not match expected result')
      results_match = False

    if not np.allclose(student_params['b2'], solution_params['b2'], atol=1, rtol=2):
      print('b2 bias did not match expected result')
      results_match = False
